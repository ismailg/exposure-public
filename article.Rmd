...
d_finished <- readRDS("data/d_finished.csv")
exposure_intervention <- paste0(as.character(d_finished %>% dplyr::select(treatment.intervention) %>% unique()) , as.character(d_finished %>% dplyr::filter(gameNumber == "11", isLastRound) %>% dplyr::select(game_opponent) %>% unique()))



d_finished <- d_finished  %>% mutate(gameNum.f = factor(gameNumber,labels = c("pre", "expo1", "expo2", "expo3","post"),levels=c("1", "11", "12", "13", "2")), 
                              condition  = exposure_intervention) %>% 
...
# First, you may need to convert the data from wide to long format
