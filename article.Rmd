...
# Find IDs for those who participated in experiment and add RS_score and high_RS as new columns to d_finished.
...
The model was estimated using the `afex` package [@singmann_afex_2022] in R. More complex models with additional random effects could not be estimated reliably, and as such the estimated model can be considered to include the optimal random effects structure [@matuschek_balancing_2017]. A similar process was used to establish the random effects structures of  linear mixed-effects models used to analyse the HMM agent investments as well as the participants' ratings of the co-players. For the $F$-tests, we used the Kenward-Roger approximation to the degrees of freedom, as implemented in the R package "afex". We Z-transform the Investment variable (subtract the overall investment mean and divide by overall standard deviation) as centering is beneficial to interpreting the main effects more easily in the presence of interactions.
...
d_finished <- d_finished  %>% mutate( gameNum.f = factor(gameNumber,labels = c("pre", "expo1", "expo2", "expo3","post"),
                                      ...
                                      investorState.f = factor(investorState, labels= c("unhappy", "neutral", "happy"), levels =c("unhappy", "neutral", "happy"))) %>% 
...
save(d_finished, file = "data/d_finished.RData")
...
# #                            Bonus Calculation
# avg_perf <- mean(temp2$perf)
# lowRS_bonus <- temp2 %>% filter(high_RS== "low RS") %>% mutate(bonus= round(0.5*perf/avg_perf,2)) %>% dplyr::select(id, bonus)
# write.csv(lowRS_bonus,"data/bonusLow.csv")
# 
# highRS_bonus <- temp2 %>% filter(high_RS== "high RS") %>% mutate(bonus= round(0.5*perf/avg_perf,2)) %>% dplyr::select(id, bonus)
# write.csv(highRS_bonus,"data/bonusHigh.csv")
...
# prolific_highRS <- read.csv(file="/Users/mbp17/Downloads/prolific_export_highRS.csv")
# 
# # Which Ids in highRS file but not in Prolific? 
# 
# # Find IDs in highRS_bonus that are not in prolific_highRS
# ids_in_highRS_not_in_prolific <- setdiff(highRS_bonus$id, prolific_highRS$Participant.id)
# 
# # Find IDs in prolific_highRS that are not in highRS_bonus
# ids_in_prolific_not_in_highRS <- setdiff(prolific_highRS$Participant.id, highRS_bonus$id)
# 
# # ids_in_highRS_not_in_prolific now contains IDs that are in highRS_bonus but not in prolific_highRS
# # ids_in_prolific_not_in_highRS now contains IDs that are in prolific_highRS but not in highRS_bonus
# 
# # print these variables to see the IDs
# print(ids_in_highRS_not_in_prolific)
# print(ids_in_prolific_not_in_highRS)
```
...
<!---------------- TRYING A MODEL WITH LATENT HMM STATE AS COVARIATE INSTEAD OF INVESTMENT  ------------------->


```{r}
mod_returns_latent <- mixed( ret_pct_na ~ phase.f*condition.f*investorState.f*high_RS + (1 + phase.f| playerId), prepost_df, REML= TRUE, method="KR")
...
# Assuming `mod_returns_latent` is your fitted model and `prepost_df` is your data frame
# Calculate estimated marginal means
emm <- emmeans(mod_returns_latent, ~ high_RS*condition.f*phase.f   | investorState.f, adjust = "tukey")

# Calculate pairwise comparisons
contrast_results <- contrast(emm, method = "pairwise")

# Extract significant contrasts
significant_contrasts <- summary(contrast_results, infer = c(TRUE, TRUE))

# Convert emmeans object to data frame for plotting
emm_df <- as.data.frame(emm)

# Plot the marginal means using ggplot2
p <- ggplot(emm_df, aes(x = investorState.f, y = emmean, color = high_RS, group = high_RS)) +
  geom_point(position = position_dodge(width = 0.5), size = 3) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), width = 0.2, position = position_dodge(width = 0.5)) +
  geom_line(position = position_dodge(width = 0.5), aes(linetype = high_RS)) +
  labs(title = "Marginal Means of Returns by Investor State and RS Group",
       x = "Investor State",
       y = "Estimated Marginal Mean of Returns") +
  theme_minimal() +
  scale_color_manual(name = "RS Group", values = c("low RS" = "blue", "high RS" = "red"))

# Add significance annotations
for(i in 1:nrow(significant_contrasts)) {
  row <- significant_contrasts[i, ]
  if(row$p.value < 0.05) {
    p <- p + annotate("text", 
                      label = "*", 
                      x = which(levels(emm_df$investorState.f) == row$investorState.f),
                      y = max(emm_df$emmean) + 0.05 * i,
                      color = "black", 
                      size = 5, 
                      vjust = -0.5)
  }
...
# Assuming `mod_returns_latent` is your fitted model and `prepost_df` is your data frame
# Calculate estimated marginal means
emm <- emmeans(mod_returns_latent, ~  high_RS | investorState.f  )
...
# Convert contrast results to data frame for annotation
contrast_df <- as.data.frame(contrast_results)
significant_contrasts <- contrast_df %>%
  filter(p.value < 0.05) %>%
  mutate(significance = ifelse(p.value < 0.001, "***", ifelse(p.value < 0.01, "**", "*")))
...
p <- ggplot(emm_df, aes(x = investorState.f, y = emmean, color = high_RS, group = high_RS)) +
  ...
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE), width = 0.2, position = position_dodge(width = 0.8)) +
  geom_line(position = position_dodge(width = 0.8), aes(linetype = high_RS)) +
  labs(title = "Marginal Means of Returns by Investor State and RS Group",
       x = "Investor State",
       y = "Estimated Marginal Mean of Returns") +
  theme_minimal() +
...
`r gsub("(\\d{1,3}),(\\d{3})", "\\1\\2", papaja::apa_print(mod_returns_pct_RS)$full_result$condition_f_inv_scaled)`, reflected  that returns were more affected by investments in the control condition. We also find a three way interaction between Phase,  Investment and RS
`r gsub("(\\d{1,3}),(\\d{3})", "\\1\\2", papaja::apa_print(mod_returns_pct_RS)$full_result$gameNum_f_condition_f_inv_scaled)`, showing that the differentiated effect of the investment on the proportion returned by RS group is itself moderated by the Phase (pre- vs post manipulation). Finally, we find a four-way interaction between Condition, Phase, Investment and RS `r gsub("(\\d{1,3}),(\\d{3})", "\\1\\2", papaja::apa_print(mod_returns_pct_RS)$full_result$phase_f_condition_f_inv_scaled_high_RS)`. For completeness, these effects are discussed in more detail the supplement.
...
<!-- To interpret this interaction we plot the effect of the Investment on returns for each level of Condition, Phase and RS. Figure \ref{fig:fourwayInteractionPlot} shows that while the effect of investment on returns is similar across RS groups and across phases in the Control condition, the Manipulation condition shows a higher effect of investment in the post phase compared to pre in the low RS group, but a lower effect of investment compared to before for the high RS group. This suggests that high RS participants based their return less on the investment after being exposed to the forgiving AI compared to the low RS group. -->
...
To test whether the HMM agent's behavior differed between Phases, Conditions and RS groups, we estimate a linear mixed-effects model of investments sent by the computerised HMM agent with Condition, Phase and RS and their interaction as fixed effects, and a similar random effects structure to the returns model. As seen in Figure \@ref(fig:boxPlots), we find no main or interaction effects, indicating HMM behavior was on aggregate similar across Phase, Conditions and RS groups.
...
<!-- We found no correlation between either the RS score and task performance, as defined by the total points accumulated in both the "pre" and "post phase of the RTG. The LPFS score was also not correlated with task performance. These two scores exhibited a positive correlation  (Rho = 0.52, p < 0.001) -->
...
Whilst we found a significant correlation between participant's Levels of Personality Functioning Score (LPFS) and the Rejection Sensitivity score (RSQ), Spearman's `r spearman_rho`, $p < 0.001$, there was no correlation between these questionnaire scores and participant's return or overall task performance.
...
Those exposed to the forgiving agent rated their opponent in the post-exposure phase lower on all attributes even though they faced the same dynamic human-like HMM as pre-exposure. One possible explanation for this drop in rating is that participants exhibited a negative contrast effect. This occurs when the evaluation of a person, object, or situation is influenced by comparisons with recently encountered contrasting objects or people. If we've recently interacted with someone exceptionally nice, our perception of a normal level of niceness might be skewed, making normal behavior seem less favourable or even negative by comparison [@kobre_negative_1972]. As the most recently faced opponents were more forgiving and cooperative, this negative contrast effect may have trumped any learning transfer from being repeatedly exposed to cooperative and forgiving AI [@zentall_within-trial_2005]. If this contrast effect is indeed replicable, then an avenue for future research would be to use it to our benefit by making the participants play agents with low cooperation perception.

<!-- Participants also perceived the forgiving AI during the exposure phase as more cooperative, but not more forgiving. The latter might be the result of the participants not having many opportunities to test the co-player's forgiveness propensity since most decided to continue cooperating. -->
...
<!-- Interestingly, for participants with low RS, the manipulation did not significantly alter their perceptions of the AI's cooperativeness, forgiveness, or their willingness to interact in the future. This group's ratings remained largely unchanged between the pre- and post-exposure phases, regardless of the AI's behavior. This stability suggests that individuals with lower sensitivity to rejection may base their ratings more on consistent behavioral patterns or other criteria not directly influenced by the forgiving nature of the AI agents. -->
...
<!-- In conclusion, this study demonstrates the value of integrating HMM-based artificial agents in the study of trust and cooperation. By showcasing the potential of these agents to both simulate realistic human interactions and influence real human behavior, our research paves the way for future investigations into the mechanisms of trust repair and the broader implications of artificial agents in social psychology. -->


<!-- ## Other notable points: -->

<!-- The study found that RSQ (RS Questionnaire) and LPFS (Levels of Personality Functioning Scale) scores were ineffective at distinguishing between high and low performers in the game. This gap highlights a need for further research, despite theoretical connections suggested in the literature review. -->
